...
import { deleteTenant, getTenantList, initTenantData, updateTenant } from '#/api/system/tenant';
...
async function onStatusChange(
  ...
  try {
    ...
    await updateTenant({ ...row, status: newStatus });
    return true;
  } catch {
    return false;
  }
...
function onInitData(row: SystemTenantApi.SystemTenant) {
  Modal.confirm({
    content: `确定要初始化租户 ${row.name} 的数据吗？此操作不可逆！`,
    title: '初始化租户数据',
    onOk: async () => {
      const hideLoading = message.loading({
      ...
      try {
        await initTenantData({ id: row.id });
        message.success({
          content: '租户数据初始化成功',
          key: 'action_process_msg',
        });
