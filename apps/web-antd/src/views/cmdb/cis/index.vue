...
const detailCiId = ref<number | undefined>(undefined);
const detailTypeId = ref<number | undefined>(undefined);
...
// 打开详情抽屉
function handleViewConfig(id: number, typeId: number) {
  detailCiId.value = id;
...
function handleAdvancedSearch(searchParams: any) {
  ...
  console.log('执行高级搜索:', searchParams);

  // 这里可以将搜索参数传递给DataTable组件
  if (dataTableRef.value && dataTableRef.value.loadDataWithParams) {
    dataTableRef.value.loadDataWithParams(searchParams);
  }
...
<template>
  <div class="cis-page">
    <div class="cis-left" :style="{ width: `${leftWidth}px` }">
      <TypeTree @select-attribute="handleTreeSelect" />
    </div>
    ...
    <!-- 高级搜索抽屉（模拟实现） -->
    <div v-if="advancedSearchVisible" class="advanced-search-mock">
      <div class="search-overlay" @click="advancedSearchVisible = false">
        <div class="search-content" @click.stop>
          <h3>高级搜索功能</h3>
          <p>
            基于
            {{ searchManager.searchConfig.defaultPageSize }} 条记录的分页搜索
          </p>
          <p>当前搜索参数: {{ JSON.stringify(currentSearchParams) }}</p>
