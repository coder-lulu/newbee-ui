...
import type { DepartmentInfo } from '#/api/system/dept/model';
...
/** 部门数据源 */
type DeptTreeArray = DepartmentInfo[];
...
async function loadTree() {
  ...
  const { data } = await deptList({ page: 1, pageSize: 10000 });
  const deptData = data;
  ...
  const ret = array2tree(processedData, { id: 'id', pid: 'parentId' });
